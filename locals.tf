locals {
  ...
  gar_repositories = [
    for gar in var.gar_repositories : {
      repo           = "${gar.region}-docker.pkg.dev/${gar.project_id}/${gar.name}"
